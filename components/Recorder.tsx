...
export default function Recorder({ onRecordingComplete }: RecorderProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 w-full max-w-md mx-auto">
      {state === 'explaining' ? (
        <div className="bg-glass-surface/90 border border-glass-border/40 backdrop-blur-xl rounded-3xl p-6 shadow-xl w-full text-center space-y-4 mb-6 transition-all duration-300">
          <h3 className="text-lg font-bold text-text-primary">Microphone Permission</h3>
          <p className="text-xs text-text-secondary leading-relaxed">
            Spill transcribes and extracts action items from your audio brain-dump.
            We need microphone access to record your thoughts. Your audio is processed securely and privately.
          </p>
          <div className="flex space-x-3 pt-2">
            <button
              onClick={() => setState('idle')}
              className="flex-1 py-2.5 px-4 bg-glass-surface border border-glass-border/40 hover:bg-accent/10 rounded-xl text-text-primary text-xs font-bold transition duration-200 cursor-pointer"
            >
              Cancel
            </button>
            <button
              onClick={startMediaRecorder}
              className="flex-1 py-2.5 px-4 bg-accent hover:bg-accent-strong text-white rounded-xl text-xs font-bold transition duration-200 shadow-md shadow-accent/25 cursor-pointer"
            >
              Allow Mic
            </button>
          </div>
      ...
      <div className="text-center mb-6 h-14 flex flex-col justify-center transition-colors duration-500">
        {state === 'recording' && (
          <div className="flex flex-col items-center space-y-1">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs font-bold text-danger uppercase tracking-wider">Recording</span>
            </div>
        ...
        {state === 'success' && (
          <span className="text-xs font-bold text-success uppercase tracking-wider flex items-center justify-center space-x-1.5">
        ...
        {state === 'error' && (
          <div className="text-center max-w-sm px-4">
            <span className="text-[10px] font-bold text-danger uppercase tracking-wider block">Error</span>
        ...
        {state === 'idle' && (
          <span className="text-xs font-medium text-text-secondary">
            Tap mic to spill your thoughts (up to 3 min)
          </span>
        )}
      </div>

      {/* Main Mic Button with Ambient Concentric Pulse */}
        ...
        >
          <button
            ...
            className={`h-36 w-36 rounded-full flex items-center justify-center transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-accent/30 ${
              state === 'recording'
                ? 'bg-gradient-to-tr from-danger to-danger/80 shadow-[0_0_30px_rgba(239,68,68,0.45)] scale-110'
                : 'bg-gradient-to-tr from-accent to-accent-strong shadow-[0_0_25px_rgba(99,102,241,0.25)] hover:shadow-accent/40'
            } disabled:opacity-40 disabled:scale-100 disabled:shadow-none cursor-pointer border-4 border-glass-border/40 backdrop-blur-md`}
      ...
      {state === 'error' && (
        <button
          onClick={() => setState('idle')}
          className="mt-5 text-xs text-accent hover:underline font-bold transition cursor-pointer"
        >
          Reset Recorder
        </button>
      )}
